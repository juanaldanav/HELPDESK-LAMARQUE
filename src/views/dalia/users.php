...
?>
<div x-data="{ openNew: false, newRole: 'sucursal' }">

<div class="flex flex-wrap items-center justify-between gap-3 mb-5">
  <div>
    <h1 class="text-[27px] font-extrabold tracking-tight">Equipo</h1>
    <p class="text-muted text-[13.5px] mt-0.5">Roles, accesos y contraseñas de los usuarios del portal.</p>
  </div>
...
<div class="grid md:grid-cols-2 xl:grid-cols-3 gap-3 stagger">
  <?php foreach ($users as $usr):
      $role = $ROLE_OF[(int)$usr['profiles_id']] ?? 'sucursal';
      $isMe = (int)$usr['id'] === (int)$me['id'];
      $dn = trim(($usr['firstname'] ?? '') . ' ' . ($usr['realname'] ?? '')) ?: $usr['name'];
      $rc = $ROLE_COL[$role]; ?>
    <div class="bg-surface rounded-card p-4 <?= (int)$usr['is_active'] ? '' : 'opacity-60' ?>" x-data="{ role: '<?= $role ?>', pw: false }">
      <div class="flex items-start justify-between gap-2 mb-3">
        <div class="flex items-center gap-2.5 min-w-0">
          <span class="w-10 h-10 shrink-0 grid place-items-center rounded-xl text-white font-extrabold text-[13px]" style="background:<?= $rc ?>">
            <?= h(mb_strtoupper(mb_substr($dn, 0, 1))) ?>
          </span>
      ...
      <?php if ($isMe): ?>
        <p class="text-[12px] text-faint font-semibold">Tu propia cuenta — pide a TI cualquier cambio.</p>
      <?php else: ?>
        <form method="post" action="<?= h(url('dalia/user/save')) ?>" class="space-y-2.5">
          ...
          <div class="flex items-center justify-between gap-2">
            ...
            <div class="flex gap-2">
              <button type="button" @click="pw = !pw" class="tap text-[12px] font-bold px-3 py-2 rounded-lg bg-canvas text-muted">Contraseña</button>
              <button class="tap text-[12px] font-bold px-3.5 py-2 rounded-lg bg-brand text-white">Guardar</button>
            </div>
          </div>
        </form>
